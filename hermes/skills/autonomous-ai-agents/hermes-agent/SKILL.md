...
4. **Vendor-prefixed model with direct provider**: If you switched from an aggregator (OpenRouter) to a direct API (DeepSeek, Anthropic, OpenAI), the model name must lose its vendor prefix. Example: `deepseek/deepseek-v4-flash` → `deepseek-v4-flash`. The gateway will say "No LLM provider configured" even though the provider is set correctly. See `references/provider-troubleshooting.md` for the diagnosis and fix commands.
